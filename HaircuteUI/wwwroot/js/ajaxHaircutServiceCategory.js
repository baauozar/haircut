$(function () {
    ...
    $('#btnSubmitHaircutCategory').on('click', function (e) {

        e.preventDefault();
        var isValid = true; // Flag to track validation
        $('#HaircutServiceCategoryAdd input[required]').each(function () {
        ...
        var formData = $('#HaircutServiceCategoryAdd').serialize();

        $.ajax({
            type: 'POST',
            url: '/Admin/HaircutServicesCategory/Create',
            data: formData,
    ...
    $(".btnEdit").on('click', function () {
        ...
        $('#editHaircutServiceCategoryId').val(id).data('original', id);
        $('#editname').val(name).data('original', name);


    });
    $("#btnEditHaircutServiceCategory").on('click', function () {

        var isValid = true; // Flag to track validation
        $('#HaircutServiceCategoryEdit input[required]').each(function () {
            if ($(this).val().trim() === '') { // Check if the field is empty
                isValid = false; // Set flag to false
                $(this).addClass('is-invalid'); // Add an invalid class for styling
                $(this).next('.invalid-feedback').show(); // Show the invalid feedback
        ...
        var formData = $('#HaircutServiceCategoryEdit').serialize();  // Serialize the form data

        // Making an AJAX request to submit the edited data
        $.ajax({
            type: 'POST',
            url: '/Admin/HaircutServicesCategory/Edit',
            data: formData,
            success: function (response) {
    ...
    $(document).on('click', '.DeleteHaircuteServiceCategory', function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/Admin/HaircutServicesCategory/Delete',
                    data: { id: Id },
                    success: function (response) {
                        if (response.success) {
    ...
    $('.bd-example-modal-lg').on('hidden.bs.modal', function () {
        ...
        $('#HaircutServiceCategoryAdd')[0].reset();

        // Remove validation styles
        $('#HaircutServiceCategoryAdd input').removeClass('is-invalid');
        $('#HaircutServiceCategoryAdd .invalid-feedback').hide();
    ...
    $('.bd-example-modal-lg-edit').on('hidden.bs.modal', function () {
        // Reset the form fields
        $('#HaircutServiceCategoryEdit')[0].reset();

        // Remove validation styles
        $('#HaircutServiceCategoryEdit input').removeClass('is-invalid');
        $('#HaircutServiceCategoryEdit .invalid-feedback').hide();
